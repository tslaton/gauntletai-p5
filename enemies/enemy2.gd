...
@export var shoot_cooldown: float = 3.0  # Time between shots (same as enemy)
@export var bullet_speed: float = 150.0  # Speed of enemy bullets (same as enemy)
@export var max_health: int = 30  # Enemy health
@export var burst_count: int = 1  # Only fires single bullets
@export var burst_delay: float = 0.15  # Not used for single shots
...
func _ready():
	...
	# Find MeshInstance3D in the enemy2 model
	var enemy2_mesh_parent = get_node_or_null("Enemy2Mesh")
	if enemy2_mesh_parent:
		for child in enemy2_mesh_parent.get_children():
...
func _apply_damage(damage: int, impact_point: Vector3, shooter: Node3D = null):
	...
	if current_health <= 0 and was_alive:
		# Award points to the shooter - enemy2 awards 10 points
