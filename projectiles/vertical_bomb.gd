...
const FIRE_WAVE_WIDTH: float = 5.0
...
func _ready():
	...
	var mesh_parent = get_node_or_null("VerticalBombMesh")
	if mesh_parent:
...
func _perform_detonation():
	...
	emit_signal("bomb_exploded", global_position, true)
	
	# Create vertical fire wave effect
	create_vertical_fire_wave()
	...
	for player in players:
		if is_instance_valid(player) and player.has_method("take_damage"):
			...
			# Check if player is in the vertical fire wave area
			# Fire wave hits all y and all z > bomb position for x +/- width
			if abs(player_pos.x - global_position.x) <= FIRE_WAVE_WIDTH and player_pos.z >= global_position.z:
...
func create_vertical_fire_wave():
	# Create a visual effect for the vertical fire wave
	# This creates a tall rectangular wave effect
	...
	process_mat.emission_box_extents = Vector3(FIRE_WAVE_WIDTH, 50.0, 1.0)  # Tall vertical box
